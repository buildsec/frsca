...
# Setup the maven trigger.
echo -e "${C_GREEN}Creating a maven example trigger: REPOSITORY=${REPOSITORY}${C_RESET_ALL}"
...
kubectl wait --timeout=5m --for=condition=ready \
  eventlisteners.triggers.tekton.dev example-maven-listener
