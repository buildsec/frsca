...
command: {
	apply: cli.Print & {
		text: yaml.MarshalStream([ for x in ssf for y in x if y.metadata.name != _|_ {y}])
	}
	create: cli.Print & {
		text: yaml.MarshalStream([ for x in ssf for y in x if y.metadata.generateName != _|_ {y}])
	}
	print: cli.Print & {
		text: json.MarshalStream([ for x in ssf for y in x {y}])
	}
}
